...
class MRIDataset3D(Dataset):
    def __init__(self, image_paths, label_paths, diag_labels, fixed_depth=32, is_train=True, is_center3=False):
        ...
        self.angle = 15
        self.final_shape = (37, 185, 250)    # center1, center2
        if is_center3: self.final_shape = (42, 181, 252)    # center3
        ...
        if self.is_train:
            ...
            self.train_transforms = Compose([
                CropForegroundd(
                    ...
                    margin=(2, 20, 20),                     # 如果想在前景外再多留一些边缘，就把 margin 设大一些
                                        allow_smaller = False
                ),
                # Replace the Pad + Center Crop combination with ResizeWithPadOrCropd
                # This transform will either pad or crop as needed to reach the target size
                ResizeWithPadOrCropd(
                    keys=["image", "label"],
...
if __name__ == '__main__':
    ...
    # Build diagnostic labels by matching against external PAS/NoPAS paths
    train_diag_labels = []
    for label_file in train_label_filenames:
        ...
        if no_pas_match:
            ...
            raise RuntimeError(f"File {label_file} not found in PAS/NoPAS directories")
    ...
    print(train_image_paths, train_diag_labels)
