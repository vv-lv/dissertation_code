...
def resample_dataset(data_dir, output_dir, target_spacing_percentile=50, anisotropy_threshold=3):
    ...
    imageTs_paths = sorted([os.path.join(data_dir, 'imagesTs_3', fname) for fname in os.listdir(os.path.join(data_dir, 'imagesTs_3'))])
    ...
    for img_path in tqdm(imageTs_paths[:]):
        img_data, img_affine = load_nifti_get_array_and_affine(img_path)
        seg_path = img_path.replace('imagesTs_3', 'labelsTs_3')
        seg_data, seg_affine = load_nifti_get_array_and_affine(seg_path)
        ...
        filename = os.path.basename(img_path)
        save_processed_image(output_dir, new_image_3d, new_affine, 'imagesTs_3', filename)

        new_label_3d = np.round(new_label_3d).astype(np.uint8)
        save_processed_segmentation(output_dir, new_label_3d, new_affine, 'labelsTs_3', filename)
