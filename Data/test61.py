...
def process_image_label_pair(image_path, label_path, output_image_dir, output_label_dir):
    ...
    label.SetSpacing(original_spacing)
    
    # 保存图像和标签
    sitk.WriteImage(processed_image, os.path.join(output_image_dir, f"{patient_id}.nii.gz"))
...
def copy_files_to_msd(matching_pairs):
    ...
    split_index = int(len(matching_pairs) * 0.7)
