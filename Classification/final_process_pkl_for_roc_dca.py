...
# --- 1. 配置区域 ---
pkl_file_paths = [
    "/home/vipuser/Desktop/test_new_file/resnet18_20250404_192331/test_results/ensemble_probs.pkl",   # 替换路径 1
    "/home/vipuser/Desktop/Classification/test_results_radiomics/MLP_test_results.pkl",  # 替换路径 2
    "/home/vipuser/Desktop/Classification/RD_test_results/predictions/InternalTest_ensemble_probs.pkl",  # 替换路径 4
]
# pkl_file_paths = [
#     "/home/vipuser/Desktop/test_new_file/resnet18_20250404_192331/test_results_3/ensemble_probs.pkl",   # 替换路径 1
#     "/home/vipuser/Desktop/Classification/test_results_radiomics_3/MLP_test_results.pkl",  # 替换路径 2
#     "/home/vipuser/Desktop/Classification/RD_test_results/predictions/ExternalTest_ensemble_probs.pkl",  # 替换路径 4
# ]
# --- 修改：为每个文件生成模型名称 ---
# 你可以手动指定，或者从路径中提取部分信息
individual_model_names = [
    "Deep Learning",    # 强调分支和双输入特性
    "Radiomics (MLP)",  # 基线对比：局部单输入
    "Fusion",   # 强调分支和双输入特性
]
...
results_dir = "/home/vipuser/Desktop/Classification/RD_test_results/predictions"
...
# y_pred_prob_ensemble = np.mean(probs_array, axis=0)
# logger.info("平均概率计算完成。")


# --- 3. 生成并保存 ROC 曲线 (比较所有单个模型) ---
logger.info("生成并保存各模型 ROC 曲线对比图...")
roc_filename = "roc_curves_comparison.png" # 新文件名
...
try:
    ...
    if first_valid_dca:
        ...
        for dca_dict in dca_results_list_for_plotting:
            if 'error' in dca_dict: # 如果是之前失败的模型
    ...
    # --- 修改：定义新的保存路径 ---
    dca_filename = "dca_curves_comparison.png" # 新文件名
    dca_save_path = os.path.join(results_dir, dca_filename)

    # --- 修改：调用绘图函数，传递所有模型名称和计算得到的DCA结果列表 ---
